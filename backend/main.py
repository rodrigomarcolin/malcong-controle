...
def validate_transfer_function(tf_input: TransferFunctionInput):
    ...
    if (len(tf_input.numerator) > 10 or len(tf_input.denominator) > 10):
        # não queremos calcular graus muito altos
        raise ValueError("O maior grau possível do numerador ou do denominador é 10.")
...
async def analyze_transfer_function(tf_input: TransferFunctionInput):
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail=f"Error analyzing transfer function: {str(e)}")
